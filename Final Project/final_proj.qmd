...
```{r}
ord_probs <- predict(polr_fit, newdata=test_baked, type="prob")
ord_class <- predict(polr_fit, newdata=test_baked, type="class")

ord_preds <- bind_cols(ord_probs, .pred_class=ord_class, test_baked)
...
ord_preds %>%
  conf_mat(truth=mental_health_status, estimate=.pred_class) %>%
  autoplot(type="heatmap") +
  labs(title="Ordinal Logit Confusion Matrix")
...
```{r}
xgb_test_preds <- bind_cols(
  predict(boost_final_fit, new_data=test, type="prob"),
  .pred_class = predict(boost_final_fit, new_data=test, type="class"),
  test
)

xgb_test_metrics <- xgb_test_preds %>%
  metrics(truth=mental_health_status, estimate=.pred_class)
...
boost_final_wf    <- boost_wf %>% finalize_workflow(best_boost_params)
...
# 1. Pull out the best RF hyperparameters by accuracy
best_rf_params <- rf_tune_res %>% select_best(metric = "accuracy")

# 2. Finalize the RF workflow with those params
rf_final_wf <- rf_wf_tune %>%
  finalize_workflow(best_rf_params)

# 3. Fit the finalized workflow on ALL of your training data
