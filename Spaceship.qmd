...
log_reg_rec <- recipe(transported ~ ., data = train) %>%
  ...
  step_rm(name) %>%
  step_rm(cabin) %>%
  step_mutate(large_food_expense = ifelse(food_court > 5000, "yes", "no")) %>%
...
```
Left diagonal 2659 & 2821 is correct prediction. Right diagonal is inaccurate. 
```{r}
log_reg_fit %>%
  augment(train) %>%
  mutate(
...
```
alternative- obtain categorical groups. Dont treat variable as numerics after. Can use this to assist with finding high, med, low income groups for competition assignment. do it like 200-400k = low. 
